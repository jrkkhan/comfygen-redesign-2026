...
// ================= CONFIGURATION =================
const STRAPI_URL = 'https://cms.comfygen.com/api/solutions'; 
...
async function fetchAllSolutions() {
  console.log(`\n🔍 Fetching all solutions from Strapi to find missing icons...`);
  try {
    const getRes = await fetch(`${STRAPI_URL}?pagination[limit]=100&populate[SolutionServiceSection][populate]=*`, {
      headers: { 'Authorization': `Bearer ${STRAPI_TOKEN}` }
    });
    const data = await getRes.json();
    return data?.data || [];
  } catch (err) {
    console.error("Failed to fetch solutions from Strapi:", err);
    return [];
  }
}
...
async function updateMissingIconsAutomatically() {
  ...
  console.log(`Found ${allSolutions.length} total solutions in Strapi.`);

  for (const entry of allSolutions) {
    const attributes = entry.attributes || entry;
    const finalSlug = attributes.slug;
    const topic = attributes.title || finalSlug;
    const updateId = entry.documentId || entry.id;
    
    const serviceSection = attributes.SolutionServiceSection?.[0];
