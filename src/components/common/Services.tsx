...
const ServiceCard = ({ title, description, link, isBlueLink = false, className = "", isClickableCard = false, children }: ServiceCardProps) => {
  const hasValidLink = !!link && link !== "#" && link.trim() !== "";
  
  if (isClickableCard) {
    const CardContent = (
      <div className={`bg-white rounded-xl border border-slate-200 p-6 sm:p-8 2xl:p-10 flex flex-col relative overflow-hidden group hover:shadow-2xl hover:shadow-blue-900/5 hover:border-primary/50 cursor-pointer transition-all duration-300 h-full ${className}`}>
        ...
        <div className="flex justify-center relative pt-4 2xl:pt-8 pb-2 2xl:pb-4 mt-auto">
  ...
  return (
    <div className={`bg-white rounded-xl border border-slate-200 p-6 sm:p-8 2xl:p-10 flex flex-col relative overflow-hidden group hover:shadow-2xl hover:shadow-blue-900/5 transition-all duration-300 h-full ${className}`}>
      <h3 className="text-balance text-base sm:text-lg 2xl:text-2xl font-semibold text-slate-900 mb-2 2xl:mb-3 !font-heading tracking-tighter ">{title}</h3>
      <p className="text-slate-500 text-xs sm:text-sm mb-4 2xl:mb-6">{description}</p>
  
      <Link href={link || "#"} className={`flex items-center text-xs sm:text-sm font-semibold mb-4 2xl:mb-8 w-fit ${isBlueLink ? 'text-primary' : 'text-slate-800 group-hover:text-primary'} transition-colors mt-auto`}>
      ...
      <div className="max-w-[1400px] mx-auto flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 2xl:gap-6 w-full mb-12 2xl:mb-16">
          ...
          <div className="flex flex-col gap-4 2xl:gap-6">
            <ServiceCard title={s0.title} description={s0.description} link={s0.link} isBlueLink={s0.isBlueLink} className={s0.className} isClickableCard={isClickableCard}>
              <IconRenderer iconProp={s0?.icon} className="w-20 h-20 text-slate-800 stroke-[1.2]" />
            </ServiceCard>

            <ServiceCard title={s1.title} description={s1.description} link={s1.link} isBlueLink={s1.isBlueLink} className={s1.className} isClickableCard={isClickableCard}>
          ...
          {/* Column 2 */}
          <div className="flex flex-col gap-4 2xl:gap-6">
            <ServiceCard title={s2.title} description={s2.description} link={s2.link} isBlueLink={s2.isBlueLink} className={s2.className} isClickableCard={isClickableCard}>
            ...
            <ServiceCard title={s3.title} description={s3.description} link={s3.link} isBlueLink={s3.isBlueLink} className={s3.className} isClickableCard={isClickableCard}>
          ...
          {/* Column 3 */}
          <div className="flex flex-col gap-4 2xl:gap-6">
            <ServiceCard title={s4.title} description={s4.description} link={s4.link} isBlueLink={s4.isBlueLink} className={s4.className} isClickableCard={isClickableCard}>
              <IconRenderer iconProp={s4?.icon} className="w-16 h-16 2xl:w-24 2xl:h-24 text-slate-800 stroke-[1.2]" />
            </ServiceCard>

            <ServiceCard title={s5.title} description={s5.description} link={s5.link} isBlueLink={s5.isBlueLink} className={s5.className} isClickableCard={isClickableCard}>
