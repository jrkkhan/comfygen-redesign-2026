...
interface IndustryModulesTabProps {
...
export const IndustryModulesTab = ({ sectionData }: IndustryModulesTabProps) => {
  const itemsToMap = (sectionData as any)?.appModuleItems || sectionData?.items;
  const finalModulesData = itemsToMap ? itemsToMap.map((m: any, idx: number) => {
    const id = m.tab || m.tabname || m.id || `tab-${idx}`;
    let icon = Smartphone;
    if (m.tabicon) {
       // Optional: map m.tabicon string to actual icon component if needed, but for now fallback logic is fine
    }
    if (id === 'vendor') icon = Store;
    ...
    return { 
      ...
      id, 
      icon,
      shortLabel: m.tabname || m.tab || m.shortLabel,
      label: m.tabname || m.tab || m.label,
      features: m.features || m.features,
      buttontext: m.buttontext || m.buttonText || "Request Full Demo",
      buttonlink: m.buttonlink || m.buttonLink || "#",
      image: getMediaUrl(m.image?.data?.[0]?.attributes?.url || m.image?.data?.attributes?.url || m.image) || null
    };
  }) : modulesData;
  ...
  const currentImage = (activeData as any).image || portfolioImages[activeIndex % portfolioImages.length];
    ...
    <section className="py-20 lg:py-28 bg-slate-50 relative overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-4 relative z-10">
        <div className="text-center max-w-4xl mx-auto mb-16 relative z-10">
          ...
          <div className="inline-flex flex-wrap justify-center items-center p-1.5 bg-slate-100/80 border border-slate-200 rounded-xl mt-10">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
          ...
          <div className="flex-1 w-full order-2 lg:order-1 relative z-20">
            ...
            <Link 
              ...
              className="mt-10 bg-slate-900 hover:bg-black text-white px-8 py-4 rounded-full font-bold transition-all hover:-translate-y-1 hover:shadow-xl shadow-slate-900/20 flex items-center gap-2 w-fit"
            >
              {activeData.buttontext || "Request Full Demo"} <ChevronRight className="w-5 h-5"/>
            </Link>
          </div>
          ...
          <div className="flex-1 w-full shrink-0 order-1 lg:order-2 relative animate-in fade-in zoom-in duration-500">
            <div className="relative w-full aspect-[4/3] rounded-[1.5rem] md:rounded-[2rem] overflow-hidden border border-slate-200">
              <Image 
                key={currentImage} // Force re-render animation when image changes
                src={currentImage}
                alt={`${activeData.label} Interface`}
                fill
                className="object-cover transition-transform duration-700 hover:scale-105 animate-in fade-in duration-500"
              />
            </div>
            {/* Background Glow */}
