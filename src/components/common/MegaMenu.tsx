...
export const MegaMenu = ({ isOpen, onClose }: MegaMenuProps) => {
  ...
  const handleSearch = () => {
    ...
    megaMenuData.solutions.forEach(c => c.links.forEach(l => allLinks.push({label: l, href: '#'})));
    megaMenuData.industries.forEach(c => c.links.forEach(l => allLinks.push({label: l, href: '#'})));
      ...
      <div className="max-w-[1600px] w-full mx-auto px-4 md:px-8 pt-6 pb-24 md:py-10 flex flex-col min-h-screen">
        ...
        {isSearching ? (
          ...
          <button 
            ...
            className={`flex items-center justify-center gap-3 p-4 md:p-6 rounded-2xl border transition-all ${
              activeTab === 'service' 
                  ...
                  return (
                    <div key={category.id} className="flex flex-col">
                      ...
                      {isActive && (
                        <div className="lg:hidden grid grid-cols-1 gap-3 pt-2 pb-4 pl-[3.75rem] pr-4">
                          {category.links.map((link, lIdx) => (
                            <a key={lIdx} href="/" className="text-[14px] text-slate-600 hover:text-primary flex items-start">
                              <ArrowRight className="w-3.5 h-3.5 mt-0.5 mr-2 text-primary opacity-70 shrink-0" />
              ...
              <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-max">
                {megaMenuData.company.map((item, idx) => (
                  <a 
                    key={idx}
                    href={item.href}
                    className={`group flex flex-col p-6 rounded-xl border transition-all ${
                      item.isActive 
