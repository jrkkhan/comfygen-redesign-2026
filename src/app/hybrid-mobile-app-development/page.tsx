...
export const metadata = {
  title: "Hybrid Mobile App Development | Comfygen",
  description: "Leading hybrid mobile app development services.",
};



async function getHybridMobileAppDevelopmentData() {
  const cmsData = await fetchAPI('/hybrid-mobile-app-development', {
    populate: {
...
export default async function HybridMobileAppDevelopmentPage() {
  const cmsData = await getHybridMobileAppDevelopmentData();
  const data = cmsData?.data || {};
  const heroSection = data?.herosection?.[0] || data?.Herosection?.[0];
  const heroTitle = heroSection?.heading || "Custom Hybrid Mobile App Development Services";

  const heroSubtitle = heroSection?.subtitle ||
    heroSection?.description?.[0]?.children?.[0]?.text ||
    "Transform your ideas into high-performance cross-platform applications. Comfygen delivers scalable, secure, and user-centric hybrid mobile apps that work flawlessly on iOS and Android platforms.";

  const servicesSection = data?.servicesection?.[0];
  const servicesTitle = servicesSection?.heading || "Hybrid Mobile App\nDevelopment Services";
  const servicesSubtitle = servicesSection?.subtitle || "Explore our comprehensive suite of hybrid application development services tailored to fuel your digital growth efficiently.";
  ...
  const aboutTitle = aboutInfoSection?.title || "Future Ready Hybrid Mobile App Development Solutions for Global Businesses";
  ...
  const aboutImageAlt = aboutInfoSection?.img?.alternativeText || "Hybrid mobile app development about info";
  ...
  let solutionTitle = "Advanced Hybrid App Solutions";
  let solutionSubtitle = "Modern hybrid apps compete on efficiency and cross-platform seamlessness. Comfygen's development integrates the best of native features with web technologies.";
  ...
  const extraCardTitle = extraCardSection?.heading || "Emerging Technologies We Leverage For Hybrid App Development";
  ...
  return (
    <main className="relative min-h-screen font-sans bg-white flex flex-col">
      <div className="relative min-h-screen overflow-hidden bg-[#02040a] flex flex-col z-[100]">
        ...
        <div className="relative z-10 flex flex-col flex-1">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <Hero
              title={
                heroTitle.includes("Hybrid Mobile App") ? (
                  <>
                    {heroTitle.split("Hybrid Mobile App")[0]}
                    Hybrid Mobile App
                    <br className="hidden md:block" />
                    {heroTitle.split("Hybrid Mobile App")[1]}
                  </>
                ) : (
                  heroTitle
                )
      ...
      <div className="flex-1">
        ...
        <CallToAction
          title={<>Ready to Dominate the <br className="hidden md:block" /> Hybrid App Market?</>}
          description="We Build Custom Hybrid Mobile Apps That Boost Orders, Retain Customers, And Help You Scale Your Business Faster."
        ...
        <Team />
        <FAQ
          title="Hybrid Mobile App Development FAQs"
          description="Have questions about our hybrid app development process? Find answers to the most commonly asked questions regarding frameworks, cost, timelines, and post-launch support."
          faqs={apiFaqs}
        />
        <Blog />
      </div>
