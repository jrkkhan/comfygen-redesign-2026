...
export const metadata = {
  title: "iOS App Development | Comfygen",
  description: "Leading iOS app development services.",
};

async function getIOSAppDevelopmentData() {
  const cmsData = await fetchAPI('/ios-app-development', {
    populate: {
...
export default async function IOSAppDevelopmentPage() {
  const cmsData = await getIOSAppDevelopmentData();
  const data = cmsData?.data || {};
  const heroSection = data?.herosection?.[0] || data?.Herosection?.[0];
  const heroTitle = heroSection?.heading || "Custom iOS App Development Services";

  const heroSubtitle = heroSection?.subtitle ||
    heroSection?.description?.[0]?.children?.[0]?.text ||
    "Transform your ideas into high-performance iOS applications. Comfygen delivers scalable, secure, and user-centric iOS mobile apps that work flawlessly on all Apple devices.";

  const servicesSection = data?.servicesection?.[0];
  const servicesTitle = servicesSection?.heading || "iOS App\nDevelopment Services";
  const servicesSubtitle = servicesSection?.subtitle || "Explore our comprehensive suite of iOS application development services tailored to fuel your digital growth efficiently.";
  ...
  const aboutTitle = aboutInfoSection?.title || "Future Ready iOS App Development Solutions for Global Businesses";
  ...
  const aboutImageAlt = aboutInfoSection?.img?.alternativeText || "iOS app development about info";
  ...
  let solutionTitle = "Advanced iOS App Solutions";
  let solutionSubtitle = "Modern iOS apps compete on efficiency and seamlessness. Comfygen's development integrates the best of native features with advanced technologies.";
  ...
  return (
    <main className="relative min-h-screen font-sans bg-white flex flex-col">
      <div className="relative min-h-screen overflow-hidden bg-[#02040a] flex flex-col z-[100]">
        ...
        <div className="relative z-10 flex flex-col flex-1">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <Hero
              title={
                heroTitle.includes("iOS App") ? (
                  <>
                    {heroTitle.split("iOS App")[0]}
                    iOS App
                    <br className="hidden md:block" />
                    {heroTitle.split("iOS App")[1]}
                  </>
                ) : (
                  heroTitle
                )
      ...
      <div className="flex-1">
        ...
        <CallToAction
          title={<>Ready to Dominate the <br className="hidden md:block" /> iOS App Market?</>}
          description="We Build Custom iOS Apps That Boost Orders, Retain Customers, And Help You Scale Your Business Faster."
        ...
        <FAQ
          ...
          description="Have questions about our iOS app development process? Find answers to the most commonly asked questions regarding frameworks, cost, timelines, and post-launch support."
          faqs={apiFaqs}
        />
        <Blog />
      </div>
